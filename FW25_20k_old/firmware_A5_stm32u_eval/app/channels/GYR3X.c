...
#define MOD_NAME "[GYRO 3D]"

#if ENABLE_GYR3X_CHAN == 1
...
void GYR3X_Init(void *p)
{
	...
	size = \
	(uint16_t)(_p->Params.sampleRate*4)*1.0/(DISP_INTERVALSEC)*sizeof(uint32_t)*GYR3X_NUM_OF_SENSORS;
	...
	else
	{
		...
		LSM6DSL_dev_GYR_deinit_sream();

		/* TODO turn off to lower power */

		char * str="\r\n[GYR3X] disabled! (deinit)\r\n";
	    aux_sendToAux(str,strlen(str),0,1,DBG_AUX);
	}
...
void GYR3X_SetActivation(void *p, bool cmd)
{
	...
	if(_p->Params.IsActive)
	{
		if(true==cmd)
		{
			if(LSM6DSL_dev_GYR_start_stream())
			{
				Error_Handler((uint8_t *)__FILE__, __LINE__);
			}
		}
		...
		{
			if(LSM6DSL_dev_GYR_stop_stream())
			{
				Error_Handler((uint8_t *)__FILE__, __LINE__);
			}
		}
