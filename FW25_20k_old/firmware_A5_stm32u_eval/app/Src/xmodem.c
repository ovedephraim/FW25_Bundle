...
uint32_t checkyheader(void)
{
	...
	memcpy(&abc[0],&aDbgRxBuffer[0],49);
	...
	_flash_file_param.f_crc2 = checksum;
	_flash_file_param.f_blocks2 = size;


	crc_received = aDbgRxBuffer[18] + (aDbgRxBuffer[19] * 0x100);

	/* We calculate it too. */
    crc_calculated = xmodem_calc_crc(&aDbgRxBuffer[1],17);
...
uint32_t get_ymodem(void)
{
  ...
  //result = uart_receive(&aDbgRxBuffer[0], 18);
...
//		  else
//		  {
//			 uart_receive(&aDbgRxBuffer[0],18);
//             HAL_Delay(2000);
//			 Transmit(X_NAK);
//		  }
