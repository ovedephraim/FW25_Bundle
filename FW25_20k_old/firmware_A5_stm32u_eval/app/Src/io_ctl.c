...
/**
 * @brief io_ctl_set_ecghpf_freq
 * @brief This function used HPF frequency
 * configuration
 * board power supply
...
void io_ctl_set_hpf_freq(ecg_hpf_conf_t f)
{
	...
	{
	case(ecg_HPF_005HZ):
			HAL_GPIO_WritePin(ECG_HPF0_GPIO_PORT, ECG_HPF0_PIN, GPIO_PIN_RESET);
			HAL_GPIO_WritePin(ECG_HPF1_GPIO_PORT, ECG_HPF1_PIN, GPIO_PIN_SET);
		break;
	case(ecg_HPF_05HZ):
			HAL_GPIO_WritePin(ECG_HPF0_GPIO_PORT, ECG_HPF0_PIN, GPIO_PIN_SET);
			HAL_GPIO_WritePin(ECG_HPF1_GPIO_PORT, ECG_HPF1_PIN, GPIO_PIN_RESET);
		break;
	case(ecg_HPF_5HZ):
			HAL_GPIO_WritePin(ECG_HPF0_GPIO_PORT, ECG_HPF0_PIN, GPIO_PIN_RESET);
			HAL_GPIO_WritePin(ECG_HPF1_GPIO_PORT, ECG_HPF1_PIN, GPIO_PIN_RESET);
...
void io_ctl_enable_power(void)
{
	...
	HAL_GPIO_WritePin(MAX30003_CS_GPIO_PORT,
			MAX30003_CS_PIN, GPIO_PIN_SET);

	HAL_GPIO_WritePin(MAX30001_CS_GPIO_PORT,
			MAX30001_CS_PIN, GPIO_PIN_SET);
...
void io_ctl_gpio_init(void)
{
  ...
  __HAL_RCC_GPIOG_CLK_ENABLE();
    ...
    /* Disconnect (PA2 - ANALOG_IN audio) from LSE Clock */
	...
	HAL_GPIO_WritePin(
			SHIELD_GND_GPIO_PORT,
			SHIELD_GND_GPIO_PIN,
			GPIO_PIN_RESET);

	HAL_GPIO_WritePin(
			CAPSENSE_EN_GPIO_PORT,
			CAPSENSE_EN_GPIO_PIN,
			GPIO_PIN_RESET);
