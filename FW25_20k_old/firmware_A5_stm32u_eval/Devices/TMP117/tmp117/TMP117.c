...
{
  TMP117_Object_t *pObj = (TMP117_Object_t *)Handle;
...
int32_t TMP117_RegisterBusIO(TMP117_Object_t *pObj, TMP117_IO_t *pIO)
{
  ...
  else
  {
    ...
    else if (pObj->IO.Init() != TMP117_OK)
    {
      ret = TMP117_ERROR;
    }
    else
...
//*/
//uint16_t TMP117_readRegister(uint8_t reg) // originally TMP117_Register reg
//{
...
//*/
//void TMP117_writeRegister(uint8_t reg, uint16_t data) // originally TMP117_Register reg
//{
...
/**
  ...
  *
  */
int32_t tmp117_read_reg(stmdev_ctx_t *ctx, uint8_t reg,
                         uint8_t *data,
                         uint16_t len)
{
  int32_t ret;
...
/**
  ...
  *
  */
int32_t ltmp117_write_reg(stmdev_ctx_t *ctx, uint8_t reg,
                          uint8_t *data,
                          uint16_t len)
{
  int32_t ret;
...
/**
  * @brief  reading device id number
  * @param  pobj - object handler
  * @param  pval - id number
  * @retval status
  *
  */
int TMP117_readId(TMP117_Object_t *pobj,
		int16_t * pval)
{
	uint16_t deviceID =0;

	if(tmp117_read_reg(&(pobj->Ctx), TMP117_DEVICE_ID, (uint8_t *)&deviceID, 2))
	{
		return TMP117_ERROR;
	}
	*pval = deviceID;
...
*/
int TMP117_readTempC(TMP117_Object_t *pobj, float * pval)
{
	int16_t digitalTempC = 0;
	int rv = tmp117_read_reg(&(pobj->Ctx), TMP117_TEMP_RESULT, (uint8_t *)&digitalTempC, 2);
...
/* READ TEMPERATURE CELSIUS
	...
	and returns the value in degrees celsius.
*/
int TMP117_readTemp(TMP117_Object_t *pobj, int16_t * pval)
{
	int16_t digitalTempC = 0;
	tmp117_read_reg(&(pobj->Ctx), TMP117_TEMP_RESULT, (uint8_t *)&digitalTempC, 2);
	*pval = digitalTempC;
	return 0;
}

#if 0 //TODO uncomment and modify
...
/* GET TEMPERATURE OFFSET
	...
	datasheet. 
*/
float TMP117_getTemperatureOffset()
{
	int16_t _offset = 0;
...
*/
void TMP117_setTemperatureOffset(float offset)
{
...
/* SET LOW LIMIT
	...
	The values are signed integers since they can be negative.
*/
void TMP117_setLowLimit(float lowLimit)
{
...
/* SET HIGH LIMIT
	...
	The values are signed integers since they can be negative.
*/
void TMP117_setHighLimit(float highLimit)
{
...
/*GET CONFIGURATION REGISTER
	...
	datasheet.
*/
uint16_t TMP117_getConfigurationRegister()
{
...
*/
uint8_t TMP117_getHighLowAlert()
{
	uint16_t configReg = 0;
...
*/
void TMP117_setAlertFunctionMode(uint8_t setAlertMode)
{
...
/*GET ALERT FUNCTION MODE
	...
	"therm" mode. This can be found on page 25 of the datasheet.
*/
uint8_t TMP117_getAlertFunctionMode()
{
	uint16_t configReg = 0;
...
*/
void TMP117_setContinuousConversionMode()
{
	uint16_t mode = 0;
...
*/
uint8_t TMP117_getConversionMode()
{
	uint16_t configReg = 0;
	configReg = readRegister(TMP117_CONFIGURATION); // Fill configReg with the configuration register
 ...
 */
void TMP117_setConversionAverageMode(uint8_t convMode)
{
	uint16_t mode = 0;
...
/*GET CONVERSION AVERAGE MODE
	...
	This can be found in the datasheet on Page 25 Table 6.
 */
uint8_t TMP117_getConversionAverageMode()
{
	uint16_t configReg = 0;
...
/* SET CONVERSION CYCLE BIT
	...
	return the cycle time accoring to the values in the chart.
       ...
       CONV  averaging  (0)     (8)     (32)   (64)
         ...
         7             16s     16s     16s      16s    
 */
void TMP117_setConversionCycleBit(uint8_t convTime)
{
	uint16_t mode = 0;
...
/* GET CONVERSION CYCLE BIT
	...
	continuous conversion mode. 
       ...
       CONV  averaging  (0)     (8)     (32)   (64)
         ...
         7             16s     16s     16s      16s    
*/
uint8_t TMP117_getConversionCycleBit()
{
	uint16_t configReg = 0;
	configReg = readRegister(TMP117_CONFIGURATION); // Fills configReg with config register values
