...
#define NVM_MAGIC_VAL (0xABABABAB)
...
#define CHANNEL_PARAM_ADDR      (FLASH_USER_START_ADDR)
#define NVM_MAGIC_VAL_ADDR      (ADDR_FLASH_PAGE_125)

typedef struct flash_param
{
	CHAN_Params_t param[CHAN_MAX];
	uint32_t magic_val;
}flash_param_t;
...
/**
  ...
  * @retval The bank of a given address
  */
static int flash_Param_Read(void * data, uint16_t dataLen, uint32_t flash_address)
{
	uint32_t *ptr = (uint32_t* )data;
	uint32_t QuadWord[4] = { 0x0,0x0,0x0,0x0 };

	QuadWord[0] = *(uint32_t *)(flash_address);
	QuadWord[1] = *(uint32_t *)(flash_address+4);
	QuadWord[2] = *(uint32_t *)(flash_address+8);
	QuadWord[3] = *(uint32_t *)(flash_address+12);

	if(QuadWord[0]!= 0xA || QuadWord[3]!= 0xA)
	{
		return SYS_ERROR_FLASH_FAILURE;
	}

	/* skip header */
	flash_address+=sizeof(QuadWord);

	for (int i = 0; i < dataLen; i+=4, ptr++, flash_address+=4 )
	{
...
}/* end of flash_Param_Read */


/**
  * @brief reads data from flash memory
  * @param data - from where to write
  * @param dataLen - bytes to write
  * @param flash_address - from where to read
  * @retval The bank of a given address
  */
static int flash_Param_Write(void * data,uint16_t dataLen, uint32_t flash_address)
{
	uint32_t QuadWord[4] = { 0xA,0xA,0xA,0xA };
	uint32_t FirstPage = 0, NbOfPages = 0, BankNumber = 0;
	FLASH_EraseInitTypeDef EraseInitStruct;
    uint32_t *pRecord = (uint32_t* )data;
    uint32_t address = flash_address;
    uint32_t PageError = 0;
    int i=0,bn=0;


    /* unlock flash access before access */
    HAL_FLASH_Unlock();
    ...
    /* program flash memory write indicator */
    if(HAL_OK!=HAL_FLASH_Program(FLASH_TYPEPROGRAM_QUADWORD, address,
			((uint32_t)QuadWord)))
    {
    	HAL_FLASH_Lock();
    	return SYS_ERROR_FLASH_FAILURE;
    ...
    address+=sizeof(QuadWord);
    bn=dataLen;

    /* write each time quad word size of data */
    for(i=0; i<dataLen; i+=4, pRecord+=4, address+=16)
    {
        if(bn >= sizeof(QuadWord))
        {
        	memcpy(QuadWord, pRecord, sizeof(QuadWord));
        	bn=bn-sizeof(QuadWord);
        }
        else
        {
        	memcpy(QuadWord, pRecord, bn);
        }
    ...
    /* lock flash access after writing */
    HAL_FLASH_Lock();

return SYS_ERROR_NONE;

}/* end of flash_Param_Write */


/**
 * @brief save_channel_params
 * @brief save parametrs to flash mem
 * 		in case when it is the first write save
 *		magic number in NVM
...
{
#if 0
	CHAN_Params_t _params[CHAN_MAX]={0};
	int32_t magic_num=0;

	/* read magic indicator value */
	flash_Param_Read(&magic_num,
				sizeof(uint32_t),NVM_MAGIC_VAL_ADDR);

	/* check if it is first read */
	if(magic_num != NVM_MAGIC_VAL)
	{
		magic_num=NVM_MAGIC_VAL;

		/* save first write magic value */
		flash_Param_Write(&magic_num,
				sizeof(uint32_t),NVM_MAGIC_VAL_ADDR);
	}

	/* read old data */
	flash_Param_Read(_params,
				sizeof(CHAN_Params_t)*CHAN_MAX,CHANNEL_PARAM_ADDR);

	_params[chan_id]=*(CHAN_Params_t*)params;

	/* write modified data */
	return flash_Param_Write(_params,
			sizeof(CHAN_Params_t)*CHAN_MAX,CHANNEL_PARAM_ADDR);
#else

	flash_param_t temp={.magic_val=NVM_MAGIC_VAL};

	/* read all stored data */
	flash_Param_Read(&temp,
				sizeof(flash_param_t),CHANNEL_PARAM_ADDR);

	temp.param[chan_id]=*(CHAN_Params_t*)params;
	temp.magic_val=NVM_MAGIC_VAL;

	/* write modified data */
	return flash_Param_Write(&temp,
			sizeof(flash_param_t),CHANNEL_PARAM_ADDR);
 ...
 * @brief restore parameters from flash mem
 * @param[out] params - parameters
 * @return - 0 on success
 */
...
int restore_channel_params(void *params)
{
	...
	/* check if it is first read */
	if(temp.magic_val != NVM_MAGIC_VAL)
	{
		return -1;
	}
	...
	{
		((CHAN_Params_t*)params)[i]=temp.param[i];
	}
...
//	}
//
//	return flash_Param_Read(params,
//			sizeof(CHAN_Params_t)*CHAN_MAX,CHANNEL_PARAM_ADDR);

}/* restore_channel_params */
