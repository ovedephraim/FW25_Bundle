/*
* @file AN_SCL.c
* @brief Analog-scl channel handler
* @author Anton Kanaev
* @version 0.0.1
* @date 30.04.2022
*
...
#define MOD_NAME "[AN SCL]"
#define CMD_BUFF_SIZE	        sizeof(CHAN_Proc_Stream_Vals_t)
#define N_CMD_BUFFERS	        (10)
#define N_DATA_BUFFERS	        (2)

#if ENABLE_AN_SCL_CHAN == 1
static fifo_t fifoStreamOut;
static int32_t	* fifoBuff;
static bool AN_SCL_move_data_Handler(void *p, void *p2,uint32_t len);

ADC_ChannelConfTypeDef an_scl_conf=
{
	.Channel      = ADC_CHANNEL_12,
	.Rank         = ADC_REGULAR_RANK_2,
	.OffsetNumber  = ADC_OFFSET_3,
	.Offset        = ADC_OFFSET_VAL
};

#endif

//uint32_t an_scl_buff[512];
MEMBUF_POOL ascl_cmdPool;
MEMBUF_POOL ascl_dataPool;


uint32_t acc_rec_len_scl=0;
...
void AN_SCL_Init(void *p)
{
	...
	if(!_p->Vals.pfifoStreamOut)
	{
		...
		/* allocate fifo buffer for this channel */
		size = \
		(uint16_t)(_p->Params.sampleRate*4)*1.0/(DISP_INTERVALSEC)*sizeof(uint32_t)*AN_SCL_NUM_OF_SENSORS;
		...
		if(initMemBufPool(&ascl_cmdPool,p,
		...
		/* Create memory pool of fixed size buffers  */
		if(initMemBufPool(&ascl_dataPool,p,
	...
	if(true ==_p->Params.IsActive)
	{
		...
		p.sn=ADC1_CH1;
		p.cb=AN_SCL_move_data_Handler;
		p.mp=&ascl_dataPool;
		p.adc_seq_hndle=(ADC_ChannelConfTypeDef*)&an_scl_conf;
		p.inv=1;

	    /* stream channel initialization */
		if(BUS_ADC1_init_and_register(&p))//ADC1_CH1))
		{
			Error_Handler((uint8_t *)__FILE__, __LINE__);
		}
	...
	{
	    /* stream channel initialization */
		if(BUS_ADC1_deinit_and_unregister(ADC1_CH1))
		{
			Error_Handler((uint8_t *)__FILE__, __LINE__);
		}

		char * str="\r\n[AN_SCL] disabled! (deinit)\r\n";
	    aux_sendToAux(str,strlen(str),0,1,DBG_AUX);
	}
...
void AN_SCL_SetActivation(void *p, bool cmd)
{
	...
	if(_p->Params.IsActive)
	{
		if(true==cmd)
		{
			...
			fifo_clear(CHAN_GetChannelfifo(CHAN_AN_SCL));
...
}/* End of AN_SCL_SetActivation */


/**
 * @brief AN_SCL_Handler
...
{
	fifo_t *an_scl_fifo=CHAN_GetChannelfifo(CHAN_AN_SCL);
	...
	acc_rec_len_scl+=((CHAN_Proc_Stream_Vals_t*)p)->num;
	/* insert time stamp in the queue */
	if (-1 == fifo_put32(an_scl_fifo,acc_rec_len_scl)){
		Error_Handler((uint8_t *)__FILE__, __LINE__);
	}

	/* insert data length in the queue */
	if (-1 == fifo_put32(an_scl_fifo,((CHAN_Proc_Stream_Vals_t*)p)->num)){
		Error_Handler((uint8_t *)__FILE__, __LINE__);
	}
	...
	for (int i=0; i<((CHAN_Proc_Stream_Vals_t*)p)->num; i++)
	{
	    ...
	    if (-1 == fifo_put32(an_scl_fifo, pbuf[i])) {
	    	Error_Handler((uint8_t *)__FILE__, __LINE__);
	    }
...
/**
 * @brief AN_SCL_move_data_Handler
...
static bool AN_SCL_move_data_Handler(void *p, void *p2,uint32_t len)
{
	...
	if(!(buff=(CHAN_Proc_Stream_Vals_t*)getMemBuf(&ascl_cmdPool))){
		Error_Handler((uint8_t *)__FILE__, __LINE__);
	}
