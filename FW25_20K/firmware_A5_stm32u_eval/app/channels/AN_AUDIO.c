/*
* @file AN_AUDIO.c
* @brief Analog-audio channel handler
* @author Anton Kanaev
* @version 0.0.1
* @date 30.04.2022
*
...
/*
* @file AN_AUDIO.c
* @brief Analog-audioiration channel handler
* @author Anton Kanaev
* @version 0.0.1
* @date 30.04.2022
*
...
#define MOD_NAME "[AN AUDIO]"
#define CMD_BUFF_SIZE	        sizeof(CHAN_Proc_Stream_Vals_t)
#define N_CMD_BUFFERS	        (10)
#define N_DATA_BUFFERS	        (2)

#if ENABLE_AN_AUDIO_CHAN == 1
static fifo_t fifoStreamOut;
static int32_t	* fifoBuff;
static bool AN_AUDIO_move_data_Handler(void *p, void *p2,uint32_t len);

ADC_ChannelConfTypeDef an_audio_conf=
{
...
//uint32_t an_audio_buff[512];
MEMBUF_POOL aaudio_cmdPool;
MEMBUF_POOL aaudio_dataPool;

uint32_t acc_rec_len_audio=0;
...
void AN_AUDIO_Init(void *p)
{
	...
	if(!_p->Vals.pfifoStreamOut)
	{
		...
		/* allocate fifo buffer for this channel */
		size = \
		(uint16_t)(_p->Params.sampleRate*3)*1.0/(DISP_INTERVALSEC)*sizeof(uint32_t)*AN_AUDIO_NUM_OF_SENSORS;
		...
		if(initMemBufPool(&aaudio_cmdPool,p,
		...
		/* Create memory pool of fixed size buffers  */
		if(initMemBufPool(&aaudio_dataPool,p,
	...
	if(true ==_p->Params.IsActive)
	{
		...
		p.sn=ADC1_CH3;
		p.cb=AN_AUDIO_move_data_Handler;
		p.mp=&aaudio_dataPool;
		p.adc_seq_hndle=(ADC_ChannelConfTypeDef*)&an_audio_conf;
		p.inv=1;

	    /* stream channel initialization */
		if(BUS_ADC1_init_and_register(&p))//ADC1_CH3))
		{
			Error_Handler((uint8_t *)__FILE__, __LINE__);
		}
	...
	else
	{
	    ...
	    /* stream channel initialization */
		if(BUS_ADC1_deinit_and_unregister(ADC1_CH3))
		{
			Error_Handler((uint8_t *)__FILE__, __LINE__);
		}

		char * str="\r\n[AN_AUDIO] disabled! (deinit)\r\n";
	    aux_sendToAux(str,strlen(str),0,1,DBG_AUX);
	}

#endif

}/* end of AN_AUDIO_Init */


/**
 * @brief AN_AUDIO_SetActivation
...
void AN_AUDIO_SetActivation(void *p, bool cmd)
{
	...
	if(_p->Params.IsActive)
	{
		if(true==cmd)
		{
			...
			fifo_clear(CHAN_GetChannelfifo(CHAN_AN_AUDIO));
...
}/* End of AN_AUDIO_SetActivation */


/**
 * @brief AN_AUDIO_Handler
...
{
	fifo_t *an_audio_fifo=CHAN_GetChannelfifo(CHAN_AN_AUDIO);
	...
	acc_rec_len_audio+=((CHAN_Proc_Stream_Vals_t*)p)->num;

	/* insert time stamp in the queue */
	if (-1 == fifo_put32(an_audio_fifo,acc_rec_len_audio)){
		Error_Handler((uint8_t *)__FILE__, __LINE__);
	}

	/* insert data length in the queue */
	if (-1 == fifo_put32(an_audio_fifo,((CHAN_Proc_Stream_Vals_t*)p)->num )){
		Error_Handler((uint8_t *)__FILE__, __LINE__);
	}
	...
	for (int i=0; i<512; i++)
	{
	    ...
	    if (-1 == fifo_put32(an_audio_fifo, pbuf[i])) {
	    	Error_Handler((uint8_t *)__FILE__, __LINE__);
	    }
	...
	return true;
}/* end of AN_AUDIO_Handler */

/**
 * @brief AN_AUDIO_Reset
 * @brief TBD
 * @returns none
 */
void AN_AUDIO_Reset()
...
}/* End of AN_AUDIO_Reset */


/**
 * @brief AN_AUDIO_move_data_Handler
...
static bool AN_AUDIO_move_data_Handler(void *p, void *p2,uint32_t len)
{
	...
	if(!(buff=(CHAN_Proc_Stream_Vals_t*)getMemBuf(&aaudio_cmdPool))){
		Error_Handler((uint8_t *)__FILE__, __LINE__);
	}

	/* fetch time stamp and other fields */
	buff->ts=*((uint32_t*)p);
	buff->id=CHAN_AN_AUDIO;
	buff->num=len;
	buff->parg=p2;
